...
/**
 * pipeline-digest.mjs — Generate a markdown digest summarizing the past 7 days.
 *
 * Numbers are computed deterministically from scan-history.tsv,
 * applications.md, fit-scores.json, and web/.liveness.json.
 * Narrative + recommended action come from Mixlayer (scorer.mixlayer in
 * config/profile.yml, same credentials as fit-score) or, without a key,
 * the local Qwen endpoint — given the numbers.
 *
 * Cache: data/digest/YYYY-MM-DD.md (Monday of that week, ISO).
 *
 * Usage:
 *   node pipeline-digest.mjs           # this week's Monday
 *   node pipeline-digest.mjs --redo    # overwrite if already exists
 *   node pipeline-digest.mjs --week 2026-04-06   # specific Monday
 */
...
function log(m) { console.log(`[${new Date().toISOString()}] pipeline-digest: ${m}`); }
...
_Generated ${new Date().toISOString()} by pipeline-digest.mjs_
